...
async function main() {
  ...
  // Get the seller address from command line arguments
  const sellerAddress = "0x70997970C51812dc3A010C7d01b50e0d17dc79C8";
  if (!sellerAddress) {
    throw new Error('Please provide a seller address as a command line argument');
  }

  // Validate the seller address
  if (!ethers.isAddress(sellerAddress)) {
    throw new Error('Invalid seller address provided');
  }
  ...
  console.log(`Fetching tickets for seller: ${sellerAddress}...`);
  
  // Call getSellerTickets function
  const sellerTickets = await ticketMarketplace.getSellerTickets(sellerAddress);
  
  console.log(`\nFound ${sellerTickets.length} tickets for seller ${sellerAddress}:`);
  console.log("============");
  
  sellerTickets.forEach((ticket, index) => {
    console.log(`\nTicket #${index + 1}:`);
